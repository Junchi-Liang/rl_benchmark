...
parser.add_argument('--n_step', type = int,
                        help = 'Number of unroll step in training',
                        default = 4)
...
if __name__ == '__main__':
    ...
    model = ActorCriticModel(**model_config)
    print('Model is ready')
    worker = ActorCriticWorker(model, env_set, args.gamma, args.lambda_gae,
        ...
        for learning_step in trange(step_total_next):
            if (it_cnt <= learning_step):
                step_sampled, finished_episode = worker.train_batch(
                            ...
                            args.coeff_entropy, learning_rate,
                            args.grad_clip_norm)
                it_cnt += step_sampled
